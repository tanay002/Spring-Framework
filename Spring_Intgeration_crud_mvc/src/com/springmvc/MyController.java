...
HibernateTemplate temp;
	...
	public ModelAndView doRegister(@ModelAttribute("user") UserDto user)
	{
		 ...
		 * AnnotationConfiguration().configure("hibernate.cfg.xml"); SessionFactory
		 * factory = cf.buildSessionFactory(); Session session = factory.openSession();
		 * Transaction t = session.beginTransaction(); session.save(user); t.commit();
		 * factory.close();
		 */ 
	...
	public  ModelAndView authenticateLoginDetail(@ModelAttribute("user") UserDto dto,HttpSession ses)
	{
		 ...
		 */
		UserDto dto1 =(UserDto) temp.get(UserDto.class,uemail);
		if(dto1!=null)
		{
	...
	{ 
		if(ses!=null)
		{
			/*
			 * AnnotationConfiguration cf = new AnnotationConfiguration();
			 * cf.configure("hibernate.cfg.xml"); SessionFactory sessionFactory =
			 * cf.buildSessionFactory(); Session session = sessionFactory.openSession();
			 */	UserDto ud= (UserDto) ses.getAttribute("dtoo");
			String email= ud.getUemail();
			UserDto dto1 =(UserDto) temp.get(UserDto.class,email);
	...
	{ 
		if(ses!=null)
		{
			 ...
			 * AnnotationConfiguration().configure("hibernate.cfg.xml"); SessionFactory
			 * factory = cf.buildSessionFactory(); Session session = factory.openSession();
			 * Transaction t = session.beginTransaction(); ses.getAttribute("dtoo");
			 */
			temp.update(dto);
			/* ((Transaction) temp).commit(); */
			/*
			  factory.close();
			 */ 
	...
	{ 
		...
		{/*
			 ...
			 */
			SessionFactory factory = temp.getSessionFactory();
